...
<div class="bg-white p-8 max-w-3xl mx-auto text-sm text-black leading-relaxed">
    ...
    <div class="mt-10 text-right">
        {{-- Jabatan --}}
        <p class="font-semibold">{{ $surat->pengirim->position }}</p>

        {{-- Tanda tangan --}}
        @if ($surat->pengirim->signature)
            <img src="{{ asset('storage/signatures/' . $surat->pengirim->signature) }}" alt="Tanda Tangan" class="w-32 mb-2 inline-block">
        @else
            <p class="italic text-gray-500">Belum ada tanda tangan diunggah</p>
        @endif

        {{-- Nama dan NIP --}}
        <p class="font-semibold">{{ $surat->pengirim->name }}</p>
    ...
    <div class="max-w-3xl mx-auto my-6">
        <div class="flex justify-between items-center">
            <a href="{{ route('admin.surat.masuk') }}" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700 shadow">
                Kembali
            </a>
