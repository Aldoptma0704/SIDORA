...
@section('content')
{{-- ✅ ADDED: Style untuk memperbaiki tinggi Trix Editor --}}
<style>
    .trix-content {
        min-height: 250px; /* Atur tinggi minimal editor */
        height: auto;      /* Biarkan tinggi bertambah jika konten panjang */
        background-color: white; /* Pastikan latar belakangnya putih */
    }
</style>

<div class="max-w-4xl mx-auto bg-white p-6 rounded shadow" x-data="{ jenis: '{{ old('jenis') }}' }">
            ...
            <div x-show="jenis === 'masuk'" x-transition>
                <div class="border-t border-gray-200 pt-4 mt-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="judul_masuk" name="judul" class="w-full mt-1 p-2 border rounded shadow-sm" :disabled="jenis !== 'masuk'">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="hidden" name="isi" id="isi_masuk" />
                    </div>
                </div>
            </div>
                ...
                <div class="border-t border-gray-200 pt-4 mt-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="nomor_surat" id="nomor_surat" class="w-full mt-1 p-2 border rounded shadow-sm" :disabled="jenis !== 'keluar'">
                        </div>
                        <div>
                            <label for="sifat" class="block text-sm font-medium text-gray-700">Sifat</label>
                            <input type="text" name="sifat" id="sifat" value="Biasa" class="w-full mt-1 p-2 border rounded shadow-sm" :disabled="jenis !== 'keluar'">
                        </div>
                        <div>
                            <label for="lampiran" class="block text-sm font-medium text-gray-700">Lampiran</label>
                            <input type="text" name="lampiran" id="lampiran" value="-" class="w-full mt-1 p-2 border rounded shadow-sm" :disabled="jenis !== 'keluar'">
                        </div>
                        <div>
                            <label for="judul_keluar" class="block text-sm font-medium text-gray-700">Hal (Perihal)</label>
                            <input type="text" name="judul" id="judul_keluar" class="w-full mt-1 p-2 border rounded shadow-sm" :disabled="jenis !== 'keluar'">
                        </div>
                    </div>

                    <div class="mt-4">
                        <label for="tujuan" class="block text-sm font-medium text-gray-700">Tujuan Surat</label>
                        <textarea name="tujuan" id="tujuan" rows="4" class="w-full mt-1 p-2 border rounded shadow-sm" placeholder="Yth. Dekan Fakultas MIPA..." :disabled="jenis !== 'keluar'"></textarea>
                    </div>

                    {{-- Isi Surat (Surat Keluar) --}}
                    <div class="mt-4">
                    <label for="isi_keluar" class="block text-sm font-medium text-gray-700">Isi Surat</label>
                        <textarea name="isi" id="editor_keluar" class="w-full border p-2 min-h-[200px]"></textarea>
                    </div>

                    <hr class="my-6">
                    <p class="text-lg font-semibold text-gray-600 mb-4">Detail Penandatangan</p>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input type="text" name="penandatangan_jabatan" placeholder="Jabatan (e.g., Plh. KEPALA DINAS,)" class="w-full mt-1 p-2 border rounded shadow-sm" :disabled="jenis !== 'keluar'">
                        <input type="text" name="penandatangan_nama" placeholder="Nama Lengkap & Gelar" class="w-full mt-1 p-2 border rounded shadow-sm" :disabled="jenis !== 'keluar'">
                        <textarea name="penandatangan_nip" placeholder="Pangkat & NIP" rows="2" class="w-full md:col-span-2 mt-1 p-2 border rounded shadow-sm" :disabled="jenis !== 'keluar'"></textarea>
                    </div>
                </div>
            </div>
